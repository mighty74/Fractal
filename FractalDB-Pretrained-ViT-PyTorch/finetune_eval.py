...
def main(cfg):
    ...
    with open(f'{os.getcwd()}/score.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        # リストのデータをCSVファイルに書き込みます
        writer.writerow(["Epoch","Averaged stats","Accuracy","Precision","Recall","F1 Score",""])
    ...
    with open(f'{os.getcwd()}/explane.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        # リストのデータをCSVファイルに書き込みます
        writer.writerow([f"{cfg.model.model_name}"])
        writer.writerow([f"{cfg.ckpt}"])
        writer.writerow([f"{cfg.path2db}"])
        writer.writerow([f"{optimizer}"])
        writer.writerow([f"{lr_scheduler}"])
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        with open(f'{os.getcwd()}/score.csv', mode='a', newline='') as file:
            writer = csv.writer(file)
            # リストのデータをCSVファイルに書き込みます
            writer.writerow([f"{epoch}", f"{metric_logger}", f"{accuracy:.5f}",f"{precision:.5f}",f"{recall:.5f}",f"{f1:.5f}",f"{all_predictions}"])
